...
def gaussian(image, seed):
    out = skimage.util.random_noise(image, mode='gaussian', seed=seed)
    out = out.astype(float32)
    #print(np.amax(out), np.amin(out))
    return out
def smooth_grad(x, y, model, times):
    lists = []
    for i in range(times):
        after_gau = []
        for img in x:
            image = torch.mul(img, 255)
            image = image.type(torch.uint8)
            image = image.numpy()
            after_gau.append(torch.from_numpy(gaussian(image, i)))
        after_gau = torch.stack(after_gau)
        lists.append(compute_saliency_maps(after_gau, y, model))
    ret = [0]*(len(x))
    for i in range(times):
        for j, img in enumerate(lists[i]):
            ret[j] += img
    for i in range(len(x)):
        ret[i] = torch.div(ret[i], times)
    return ret    
