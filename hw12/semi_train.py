...
source_dataset = ImgDataset(source_transform, target_transform, './real_or_drawing', train = True)
print('dataset done')
target_dataset = ImgDataset(source_transform, target_transform, './real_or_drawing', train = False)
...
for epoch in range(1000):
    train_D_loss, train_F_loss, train_acc = train_epoch(source_dataloader, target_dataloader, lamb=((2/(1+math.exp(-10*epoch/999))) - 1))
    if (epoch + 1) % 100 == 0:
        torch.save(feature_extractor.state_dict(), f'_semi_lamb_extractor_model_{epoch+1:03d}.bin')
        torch.save(label_predictor.state_dict(), f'_semi_lamb_predictor_model_{epoch+1:03d}.bin')
