...
def train_epoch(source_dataloader):
    ...
    for i, (source_data, source_label) in enumerate(source_dataloader):
        ...
        class_logits = label_predictor(feature_extractor(source_data))
...
for epoch in range(1000):
    ...
    if (epoch + 1) % 100 == 0:
        torch.save(feature_extractor.state_dict(), f'noDANN_extractor_{epoch+1:03d}.bin')
        torch.save(label_predictor.state_dict(), f'noDANN_predictor_{epoch+1:03d}.bin')
        #torch.save(domain_classifier.state_dict(), f'noDANN_{epoch+1:03d}.bin')
